"""
Books management GUI module (SQLite-compatible)
"""
...
from Core.validators import validate_book_fields
...
class BookmanagementWindow:
    ...
    def search_books(self):
        ...
        like = f"%{keyword}%"
        query = """SELECT * FROM books WHERE title LIKE ? OR author LIKE ? OR isbn LIKE ? OR publisher LIKE ? OR category LIKE ?"""
    ...
    def open_book_form(self, mode, book_id=None, values=None):
        ...
        status = ttk.Combobox(form, width=30, values=["Available", "Borrowed", "Reserved", "Maintenance"])
        ...
        def save():
            ...
            if mode == "add":
                ...
                query = """UPDATE books SET isbn=?,title=?,author=?,publisher=?,publication_year=?,
                           category=?,location=?,quantity=?,status=? WHERE book_id=?"""
                params = (isbn.get(), title.get(), author.get(), publisher.get(), year.get(),
    ...
    def delete_book(self):
        ...
        if messagebox.askyesno("Confirm Delete", f"Delete book '{title}'?"):
            db.execute_query("DELETE FROM books WHERE book_id=?", (selected[0],))
            self.load_books()
