...
try:
    from Core.email_utils import send_book_available_notification
except Exception:
...
class BorrowManagementWindow:
    def __init__(self, master, user_data=None, dashboard_root=None):
        ...
        tk.Label(header,
                 text=f"Logged in as: {self.user_data.get('first_name','')} {self.user_data.get('last_name','')}",
    ...
    def borrow_dialog(self):
        ...
        # Fetch available books (quantity > 0 and status = 'Available')
        books = db.execute_query(
            "SELECT book_id, title, quantity FROM books WHERE quantity > 0 AND status = 'Available'",
            fetch=True
        )
        ...
        def save_borrow():
            ...
            active_res = db.execute_query(
                ...
                SELECT r.student_id
                FROM reservations r
                WHERE r.book_id = ? AND r.status = 'Ready'
            ...
            db.execute_query(
                ...
                INSERT INTO borrow_transactions
                    (student_id, book_id, librarian_id, borrow_date, due_date, status)
                VALUES (?, ?, ?, ?, ?, 'Active')
                """,
            ...
            # Reduce quantity and update status if needed
            db.execute_query(
                """
                UPDATE books
            ...
            if active_res:
                db.execute_query(
                    "UPDATE reservations SET status = 'Completed' WHERE book_id = ? AND student_id = ?",
                    (book_id, student_id),
                    fetch=False
                )
    ...
    def load_records(self):
        ...
        # Auto-mark overdue
        db.execute_query(
            "UPDATE borrow_transactions SET status = 'Overdue' WHERE status = 'Active' AND DATE(due_date) < DATE('now')",
            fetch=False
        )
    ...
    def search_records(self):
        ...
        query = """
            ...
            JOIN students s ON bt.student_id = s.student_id
            JOIN books b ON bt.book_id = b.book_id
            WHERE LOWER(s.first_name) LIKE ? OR LOWER(s.last_name) LIKE ? OR LOWER(b.title) LIKE ?
            ORDER BY bt.transaction_id DESC
        """

        key = f"%{keyword}%"
        results = db.execute_query(query, (key, key, key), fetch=True) or []
    ...
    def go_back(self):
        """Close this window and return to dashboard if provided."""
        self.master.destroy()
